...
def write_utf8_copies(extracted_root, localization_root):
    ...
    with inventory.open("w", encoding="utf-8-sig", newline="") as handle:
        writer = csv.DictWriter(handle, fieldnames=TEXT_INVENTORY_FIELDS)
        writer.writeheader()
...
def build_scenario_table(extracted_root, localization_root):
    ...
    if not files:
        raise FileNotFoundError(
            f"未找到反编译后的剧情 JSON: {scenario_root}/*.txt.json\n"
            "请检查 extracted 中是否存在 SCN，以及 SCN 反编译步骤是否成功。"
        )
    if not rows:
        raise ValueError(
            f"已反编译 {len(files)} 个剧情 JSON，但没有可导出的文本段。"
        )

    table = localization_root / "scenario_dialogue_zh_cn.tsv"
    with table.open("w", encoding="utf-8-sig", newline="") as handle:
        writer = csv.DictWriter(handle, fieldnames=SCENARIO_TABLE_FIELDS, delimiter="\t")
        writer.writeheader()
        writer.writerows(rows)
