...
def main():
    ...
    with archive.open("rb") as stream:
        ...
        for index, record in enumerate(records, 1):
            ...
            if size_ok and adler_ok and record["adler32"] != 0:
                status = "verified"
            elif size_ok and record["adler32"] == 0:
                # Some third-party XP3 repackers deliberately omit per-file
                # Adler-32 values. Preserve that distinction: the payload has
                # a verified length, but no archive checksum was available.
                status = "unverified_no_adler32"
            ...
            if status == "verified":
                verified += 1
            elif status == "unverified_no_adler32":
                unverified_no_adler32 += 1
    ...
    summary = {
        ...
        "files_unverified_no_adler32": unverified_no_adler32,
