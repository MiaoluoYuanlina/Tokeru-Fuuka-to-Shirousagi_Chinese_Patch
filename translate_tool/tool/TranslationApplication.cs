...
internal static class TranslationApplication
{
    public static async Task<int> RunAsync(string[] args)
    {
        ...
        try
        {
            ...
            {
                return ProviderProtocolSelfTest.Run();
            }
            ...
            var options = AppOptions.FromGlobalSettings();
            Console.WriteLine(
                $"全局配置：SourceHeader={options.SourceHeader}，TargetHeader={options.TargetHeader}，" +
                $"HeaderRow={options.HeaderRow}，HeaderStartColumn={options.HeaderStartColumn}");
            Console.WriteLine(
                $"API 配置：Provider={options.Provider}，Protocol={options.ProtocolDisplayName}，" +
                $"Url={options.ApiUrl}，Model={options.Model}");

            var settingsDirectory = AppContext.BaseDirectory;
            var outputPath = ResolveOutputPath(commandLine, options, inputPath, settingsDirectory);
            var cachePath = ResolveRelativePath(options.CacheFile, settingsDirectory);
            ...
            {
                using var translator = new MultiProviderTranslator(options);
    ...
    private static string ResolveOutputPath(CommandLineOptions commandLine, AppOptions options, string inputPath, string settingsDirectory)
    {
        ...
        {
            return ResolveRelativePath(options.OutputFile, settingsDirectory);
        }
    ...
    private static async Task TranslatePendingAsync(
        ...
        int totalTexts,
        MultiProviderTranslator translator,
        TableDocument table,
        int targetColumn,
